...
func TestCalVer_NoTagsToday_ReturnsN1(t *testing.T) {
	s := newTestScheme([]string{
		...
		"202505.1.5",  // different year
		"202605.31.3", // yesterday (May 31)
	})
	v, err := s.NextVersion(versioning.ReleaseContext{RepoPath: "."})
	require.NoError(t, err)
	assert.Equal(t, "202606.1.1", v)
}
...
func TestCalVer_ExistingTagsToday_IncrementsN(t *testing.T) {
	...
	v, err := s.NextVersion(versioning.ReleaseContext{RepoPath: "."})
	require.NoError(t, err)
	assert.Equal(t, "202606.1.4", v)
}

func TestCalVer_OnlyUnrelatedTags_ReturnsN1(t *testing.T) {
	s := newTestScheme([]string{"v1.0.0", "v2.3.4", "202606.2.1"})
	v, err := s.NextVersion(versioning.ReleaseContext{RepoPath: "."})
	require.NoError(t, err)
	assert.Equal(t, "202606.1.1", v)
}
...
func TestCalVer_NonNumericSuffix_Ignored(t *testing.T) {
	s := newTestScheme([]string{"202606.1.alpha", "202606.1.1"})
	v, err := s.NextVersion(versioning.ReleaseContext{RepoPath: "."})
	require.NoError(t, err)
	assert.Equal(t, "202606.1.2", v)
}
...
func TestCalVer_Validate(t *testing.T) {
	...
	cases := []struct {
		...
		{"202606.11.1", true},
		{"202606.11.42", true},
		{"202606.1.1", true},     // day without leading zero
		{"202606.11.0", false},   // N must be > 0
		{"202606.11", false},     // missing N
		{"v202606.11.1", false},  // prefix not valid
		{"2026.06.11.1", false},  // old 4-part format no longer valid
		{"1.2.3", false},
		{"", false},
	}
